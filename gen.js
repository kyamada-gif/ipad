...
function finalOf(kind, ease) {
  ...
  if (kind === "mask") {
    ...
    return mk({
      ...
      steps: [
        { t: `/${len} を 8 で区切る`, v: `${Math.floor(len / 8)} 個そろって、余り ${len % 8}` },
        { t: "答え", v: maskStr(len) },
      ],
...
/* ── 6ステージ 必要なサブネット数 → サブネットマスク ────────────
   5ステージと同じ手順。基準（クラスのマスク）から、必要なビット分延ばす。
...
GEN.S6 = (ease) => {
  ...
  const want = pick((ease != null && ease < 0.45 ? EASY : HARD).filter((n) => bitsFor(n) <= maxBits));
  const bits = bitsFor(want);
  const len = base + bits;
  return {
    station: "S6",
    given: [{ k: "使うアドレス", v: `クラス${cls}（/${base} から）` }, { k: "必要なサブネット数", v: `${want} 個` }],
    prompt: "このサブネット数が作れるサブネットマスクはどれですか。",
...
GEN.S7 = (ease) => {
  ...
  return {
    ...
    prompt: "この範囲を1つのアドレスにまとめるとどれですか。",
    input: "stack",
...
const GROUPS = [
  { at: "S0", name: "基礎",
    note: "2進数と10進数の行き来。ステージ5から先で、ずっと使います。" },
  { at: "S3", name: "試験レベル",
    note: "本番で問われる形。ステージ1〜4で覚えたことを組み合わせます。" },
  { at: "SF", name: "仕上げ",
    note: "どのステージのやり方を使うかを、自分で決めて解きます。" },
];
GROUPS.forEach((g, i) => {
...
const WAY = {
  ...
  S6: { h: "必要なサブネット数が入る、いちばん小さい 2の◯乗を選びます",
    b: "クラスごとに出発点が決まっています。A は /8、B は /16、C は /24 です。1桁延ばすごとに、作れるサブネットの数は2倍になります。必要な数が入るいちばん小さい 2の◯乗を選ぶと、その◯が延ばす桁数です。台数のときと違って、2 は足しません。" },
...
const LINK = {
  ...
  S6: "「必要な数が入る、いちばん小さい 2の◯乗」を探すのは前のステージと同じです。今度は 2 を足しません。",
...
const EXAMPLES = {
  ...
  S6: { title: "クラスB で10個のサブネット", rows: [
    ...
    ["クラスB の出発点から", "/16 + 4 = /20"],
    ["答え", "255.255.240.0　/20"]],
    note: "教材は 10 に 2 を足しているが、いまの試験では足さない。" },
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = { STATIONS, GROUPS, EXAMPLES, FINAL_KINDS, finalOf, WAY, LINK, NEXT, FIGURE, GEN, addrWith, pairOut, restOnes, stepRounds, makeQuestion, powTable, wrongsOf, splitOut, pickOut, stackOut, maskBoardOut,
