...
function tipOf(v) {
  ...
  if (M[v]) return `${v} は「上から ${M[v]} 個だけ 1」。サブネットマスクに出る値はこの8通りしかない。`;
  return null;
}
...
GEN.S8 = (ease) => {
  ...
  return {
    ...
    steps: [
      ...
      { t: "8 個で 255 が1つできる", v: `${len} ÷ 8 = ${full} あまり ${rest}` },
      { t: rest ? "あまりは、上の桁から 1 にする" : "あまりが無いので、残りは 0", v: mask },
      { t: "答え", v: goal === "toMask" ? mask : `/${len}` },
    ],
    tip: "サブネットマスクに出る数は 128 192 224 240 248 252 254 255 の8つだけ。上から順に 1 が増えていく。",
  };
};
...
const HOW = {
  ...
  S8: "/28 と 255.255.255.240 は同じ意味です。この表を、両向きに即答できるようにします。",
  S3: "サブネットマスクの 255 でない数を2進数にして、1 が終わるところで区切ります。IPアドレスの同じところで、区切りより右をぜんぶ 0 にするとネットワークアドレス、ぜんぶ 1 にするとブロードキャストアドレスです。",
...
const LINK = {
  ...
  S8: "240 = 11110000 は、前のステージの引き算で作れました。ここでは結果を丸暗記して、計算そのものを無くします。",
...
const NEXT = {
  ...
  S2: "つぎのステージの 255 や 240 は、この引き算で 1 の数に直せます。次からは結果を丸暗記して、計算を消します。",
...
/** ステージごとの**丸暗記カード**。閉じた集合。ここを覚えれば、その先の思考が消える。
 *  練習は、この山を**表なしで即答**できるまで回す。 */
/** 暗記ドリルを置くステージと、そのカード。**本番で問われる形だけ。**
 *  置くのは「閉じた集合」があるステージだけ。ルールが1行しかないステージには置かない。 */
const CARDS = {
  // ① 2の◯乗
  S0: [2, 3, 4, 5, 6, 7, 8, 9, 10].map((n) => ({ q: `2の${n}乗`, a: String(Math.pow(2, n)),
    w: n <= 3 ? Array(n).fill("2").join(" × ") + ` = ${Math.pow(2, n)}` : `${Math.pow(2, n - 1)} の2倍` })),
  // ② プレフィックス長 ↔ サブネットマスク（第4オクテットの山と、第3オクテットの山）
  S8: [25, 26, 27, 28, 29, 30, 31, 32].map((n) => ({ q: `/${n}`, a: maskStr(n),
    w: `${n} = 8×3 あまり ${n - 24} → 1 が ${n - 24} 個で ${256 - Math.pow(2, 32 - n)}` }))
    .concat([25, 26, 27, 28, 29, 30].map((n) => ({ q: maskStr(n), a: `/${n}`,
      w: `255 が3つで24個、${256 - Math.pow(2, 32 - n)} は 1 が ${n - 24} 個` })))
    .concat([17, 18, 19, 20, 21, 22, 23].map((n) => ({ q: `/${n}`, a: maskStr(n),
      w: `${n} = 8×2 あまり ${n - 16} → 3つ目に ${256 - Math.pow(2, 24 - n)}` })))
    .concat([17, 18, 19, 20, 21, 22, 23].map((n) => ({ q: maskStr(n), a: `/${n}`,
      w: `255 が2つで16個、${256 - Math.pow(2, 24 - n)} は 1 が ${n - 16} 個` }))),
  // ④ プレフィックス長 → 使えるホスト数
  S5: [30, 29, 28, 27, 26, 25, 24, 23, 22].map((n) => ({ q: `/${n}`, a: `${Math.pow(2, 32 - n) - 2} 台`,
    w: `2の${32 - n}乗 − 2 = ${Math.pow(2, 32 - n) - 2}` })),
  // ⑤ サブネットマスク → ワイルドカードマスク（別枠。ACL と OSPF で使う）
  S9: [128, 192, 224, 240, 248, 252, 254, 255].map((v) => ({ q: `サブネットマスクが ${v}`, a: String(255 - v),
    w: `255 − ${v} = ${255 - v}` })),
};
...
/* 教材に載っている見本。数字は教材のまま。読んだ直後に「これだ」とつながるように。 */
const EXAMPLES = {
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = { STATIONS, EXAMPLES, HOW, LINK, NEXT, EXLINE, CARDS, GEN, addrWith, pairOut, makeQuestion, wrongsOf, splitOut, pickOut, stackOut, maskBoardOut,
