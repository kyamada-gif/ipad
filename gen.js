...
GEN.S6 = (ease) => {
  ...
  return {
    ...
    /* **注記は置かない。**前はここに「教材では 2 を足しているが、それは昔の決まり」と出していたが、
       正解した直後に「別の数え方もある」と言われると、覚えたばかりの手が揺れる。
       このアプリは 2 を足さない。**正しいやり方だけを載せる。** */
    tip: null,
  };
};
...
const GROUPS = [
  { at: "S0", name: "基礎", note: "試験レベルの問題を解く土台になります。",
    /* ── まとまりの最後に置く、押すと開くブロック ────────────────
       **解き方の代わりではない。**教材の手順（1 にした桁の数を全部足す）で
       解けるようになったうえで、よく出る並びを覚えておくと、その足し算を
       省ける、という位置づけ。だから解き方の帯にも、説明の1枚にも混ぜない。
       まとまりを解き終えた場所に、閉じた状態で置く。 */
    tip: {
      label: "2進数 → 10進数の変換を楽にするコツ",
...
GROUPS.forEach((g, i) => {
  ...
  // まとまりの**最後**のステージ。tip はこの札の下に出す
  g.endAt = STATIONS[g.to - 1].id;
});
